            ...
            <tr>
                <th class="text-center" style="border-color: #9d9d9d">
                    <input type="text" value="{{ $m->nombre }}" style="width: 100%"
                        id="nombre_motivo_{{ $m->id_motivo_baja }}"
                        class="padding_lateral_5 {{ !$m->estado ? 'error' : '' }}">
                </th>
                <th class="text-center" style="border-color: #9d9d9d">
                    <div class="btn-group">
                        <button type="button" class="btn btn-xs btn-yura_warning"
                            onclick="update_motivo('{{ $m->id_motivo_baja }}')">
                            <i class="fa fa-fw fa-edit"></i>
                        </button>
                        <button type="button" class="btn btn-xs btn-yura_danger"
                            onclick="cambiar_estado_motivo('{{ $m->id_motivo_baja }}')">
                            <i class="fa fa-fw fa-{{ $m->estado ? 'lock' : 'unlock' }}"></i>
                        </button>
                    </div>
                </th>
            </tr>
...
<script>
    ...
    function store_motivos() {
        ...
        if (data.length > 0) {
            ...
            post_jquery_m('{{ url('botar_inventario/store_motivos') }}', datos, function() {
                cerrar_modals();
                admin_motivos();
            })
        }
    ...
    function update_motivo(id) {
        ...
        if (nombre != '') {
            ...
            post_jquery_m('{{ url('botar_inventario/update_motivo') }}', datos, function() {});
        }
    }
    ...
    function cambiar_estado_motivo(id) {
        ...
        post_jquery_m('{{ url('botar_inventario/cambiar_estado_motivo') }}', datos, function() {
            cerrar_modals();
            admin_motivos();
        });
    }
