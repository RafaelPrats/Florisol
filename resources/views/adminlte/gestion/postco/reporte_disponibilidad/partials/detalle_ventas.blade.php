        ...
        @foreach ($listado as $pos_p => $item)
            <tr onmouseover="$(this).addClass('bg-yura_dark')" onmouseleave="$(this).removeClass('bg-yura_dark')">
                <th class="padding_lateral_5" style="border-color: #9d9d9d">
                    {{ convertDateToText($item['postco']->fecha) }}
                </th>
                <th class="padding_lateral_5" style="border-color: #9d9d9d">
                    @foreach ($item['clientes'] as $cli)
                        <p>-{{ $cli->cliente->detalle()->nombre }}</p> <sup class="hidden">{{ $cli->id_cliente }}</sup>
                    @endforeach
                </th>
                <th class="padding_lateral_5" style="border-color: #9d9d9d">
                    {{ $item['postco']->variedad->nombre }} <sup class="hidden">{{ $item['postco']->id_postco }}</sup>
                </th>
                <th class="text-center bg-yura_dark">
