<legend class="text-center" style="font-size: 1em; margin-bottom: 5px">
    Distribucion de "<b>{{ $det_pedido->ramos }}</b>" ramos de "<b>{{ $det_pedido->longitud }}cm</b>" en la receta
    "<b>{{ $det_pedido->variedad->nombre }}</b>"
    para el pedido "<b>{{ $det_pedido->pedido->codigo }}</b>"
</legend>
<input type="hidden" id="id_detalle_pedido_seleccionado" value="{{ $det_pedido->id_detalle_import_pedido }}">
<input type="hidden" id="ramos_pedido" value="{{ $det_pedido->ramos }}">
<input type="hidden" id="longitud_pedido" value="{{ $det_pedido->longitud }}">
    ...
    <tr>
        ...
        <td>
            ...
            @else
                ...
                <button type="button" class="btn btn-block btn-yura_primary" onclick="store_distribucion_receta()">
                    <i class="fa fa-fw fa-save"></i> Grabar
                </button>
                <button type="button" class="btn btn-block btn-yura_warning" onclick="reiniciar_distribucion_receta()">
                    <i class="fa fa-fw fa-refresh"></i> Reiniciar Dist.
                </button>
            @endif
        </td>
            ...
            <div style="overflow-y: scroll; overflow-x: scroll; max-height: 500px;">
                <table class="table-striped table-bordered" style="width: 100%; border: 1px solid #9d9d9d"
                    ...
                    @php
                        $pos = 0;
                    @endphp
                    @foreach ($det_pedido->detalles_receta as $pos => $item)
...
<script>
    ...
    function store_distribucion_receta() {
        ...
        if (data.length > 0) {
            ...
            post_jquery_m('{{ url('importar_pedidos/store_distribucion_receta') }}', datos, function(retorno) {
                cerrar_modals();
                admin_receta(id_detalle_pedido);
            });
        }
    }

    function reiniciar_distribucion_receta() {
        id_detalle_pedido = $('#id_detalle_pedido_seleccionado').val();
        datos = {
            _token: '{{ csrf_token() }}',
            id_detalle_pedido: id_detalle_pedido,
        };
        post_jquery_m('{{ url('importar_pedidos/reiniciar_distribucion_receta') }}', datos, function(retorno) {
            cerrar_modals();
            admin_receta(id_detalle_pedido);
        });
    }
