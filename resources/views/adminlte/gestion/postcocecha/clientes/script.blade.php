<script>
    ...
    function buscar_listado() {
        ...
        $.get('{{ url('clientes/buscar') }}', datos, function(retorno) {
            ...
            $('#table_content_clientes_filter>label>input').addClass('input-yura_default')
        }).always(function() {
            $.LoadingOverlay('hide');
        });
    }

    function add_importar_clientes() {
        datos = {}
        get_jquery('{{ url('clientes/importar_clientes') }}', datos, function(retorno) {
            modal_view('modal_importar_clientes', retorno, '<i class="fa fa-fw fa-plus"></i> Importar Clientes',
                true, false, '{{ isPC() ? '95%' : '' }}',
                function() {});
        })
    }
    ...
    function importar_clientes() {
        if ($('#form-importar_clientes').valid()) {
            $.LoadingOverlay('show');
            formulario = $('#form-importar_clientes');
                ...
                success: function(retorno2) {
                    if (retorno2.success) {
                        ...
                        alerta_accion(retorno2.mensaje, function() {
                            ...
                            get_jquery('{{ url('clientes/get_importar_clientes') }}', datos,
                                function(retorno) {
                                    $('#div_importar_clientes').html(retorno);
                                });
                        });
                    } else {
