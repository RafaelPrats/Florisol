...
@if ($total_venta != $total_inventario)
    <script>
        actualizar_variedad();

        function actualizar_variedad() {
            $('#div_detalles_venta').html(
                '<div class="alert alert-success text-center"><i class="fa fa-fw fa-refresh"></i> ACTUALIZANDO DATOS DE VENTA</div>'
            );
            datos = {
            ...
            get_jquery('{{ url('inventario_compra_flor/actualizar_variedad') }}', datos, function(retorno) {
                cerrar_modals();
                $('#tr_' + datos['variedad']).html(retorno);
                detalle_ventas(datos['variedad'], '{{ $total_venta }}');
            });
        }
    </script>
@endif
