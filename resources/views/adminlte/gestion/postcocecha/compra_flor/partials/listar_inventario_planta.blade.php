<td colspan="3">
    <table class="table-bordered" style="width: 100%; border: 1px solid #9d9d9d">
        ...
        @foreach ($listado as $pos => $item)
            @php
                $total_inventario_planta += $item->tallos_x_malla;
            @endphp
            <tr style="background-color: {{ $pos % 2 == 0 ? '#dddddd' : '' }}">
                ...
                <th class="text-center" style="border-color: #9d9d9d">
                    <input type="number" id="tallos_x_malla_inv_{{ $item->id_desglose_compra_flor }}" min="0"
                        value="{{ $item->tallos_x_malla }}" style="width: 100%" class="text-center">
                </th>
                <th class="text-center" style="border-color: #9d9d9d">
                    <div class="btn-group">
                        <button type="button" class="btn btn-xs btn-yura_warning"
                            onclick="update_inventario('{{ $item->id_desglose_compra_flor }}', '{{ $item->id_planta }}')">
                            <i class="fa fa-fw fa-pencil"></i>
                        </button>
                        <button type="button" class="btn btn-xs btn-yura_danger"
                            onclick="delete_inventario('{{ $item->id_desglose_compra_flor }}', '{{ $item->id_planta }}')">
