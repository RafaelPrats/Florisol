<legend class="text-center" style="font-size: 1em; margin-bottom: 5px">
    <div class="input-group">
        <span class="input-group-addon bg-yura_dark span-input-group-yura-fixed">
            Distribucion de "<b>{{ $detalle_pedido->variedad->nombre }}</b>"
            para el pedido "<b>{{ $detalle_pedido->pedido->codigo }}</b>"
        </span>
        <select id="select_numero_receta" class="form-control" style="width: 100%">
            @foreach ($numeros_receta as $n)
                <option value="{{ $n }}">{{ $n }}</option>
            @endforeach
        </select>
        <span class="input-group-btn">
            <button type="button" class="btn btn-yura_primary" onclick="cargar_receta()" style="border-radius: 0">
                <i class="fa fa-fw fa-arrow-left"></i> Cargar esta receta
            </button>
        </span>
        <span class="input-group-addon bg-yura_dark">
            Ramos a Dividir
        </span>
        <input type="number" id="input_dividir_ramos" class="form-control text-center" style="width: 100%"
            placeholder="Ramos a Dividir">
    </div>
</legend>
<input type="hidden" id="id_detalle_pedido_seleccionado" value="{{ $detalle_pedido->id_detalle_import_pedido }}">
<input type="hidden" id="ramos_pedido" value="{{ $detalle_pedido->ramos }}">
<input type="hidden" id="longitud_pedido" value="{{ $detalle_pedido->longitud }}">
            ...
            <div style="overflow-y: scroll; overflow-x: scroll; max-height: 500px;">
                <table class="table-striped table-bordered" style="width: 100%; border: 1px solid #9d9d9d"
                    ...
                    <tbody id="tbody_variedades_seleccionados">
                        @foreach ($detalle_pedido->detalles_receta as $pos => $item)
...
<script>
    ...
    function buscar_variedades() {
        ...
        get_jquery('{{ url('importar_pedidos/buscar_variedades') }}', datos, function(retorno) {
    ...
    function store_dividir_receta() {
        cant_variedad_seleccionado = $('.cant_variedad_seleccionado');
        data = [];
        id_detalle_pedido = $('#id_detalle_pedido_seleccionado').val();
        ...
        if (data.length > 0) {
            datos = {
                ...
                fecha: $('#vista_actual').val() == 'ingreso_clasificacion' ? $('#fecha_filtro').val() : $('#fecha_receta_armar').val(),
                ramos: $('#input_dividir_ramos').val(),
                data: JSON.stringify(data)
            };
            post_jquery_m('{{ url('ingreso_clasificacion/store_dividir_receta') }}', datos, function(retorno) {
                cerrar_modals();
                if ($('#vista_actual').val() == 'ingreso_clasificacion')
                    armar_combinacion($('#id_receta_armar').val(), $('#longitud_receta_armar').val(), $(
                        '#fecha_receta_armar').val());
                else
                    modal_planificacion($('#pos_selected').val(), $('#fecha_receta_armar').val());
            });
        }
    }
    ...
    function cargar_receta() {
        ...
        get_jquery('{{ url('importar_pedidos/cargar_receta') }}', datos, function(retorno) {
