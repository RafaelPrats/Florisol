...
class cronUpdateResumenAgrogana extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'resumen:update {fecha=0} {variedad=0} {dev=0}';
     ...
     * @var string
     */
    protected $description = 'Comando para calcular los valores resumidos de Agrogana';
    ...
    public function handle()
    {
        ...
        if ($dev == 0) {
            dump('<<<<< ! >>>>> Ejecutando comando "resumen:update" <<<<< ! >>>>>');
            Log::info('<<<<< ! >>>>> Ejecutando comando "resumen:update" <<<<< ! >>>>>');
        }
        ...
        foreach ($fechas as $pos_f => $fecha) {
            if ($variedad != 0) {
                $variedades = [getVariedad($variedad)];
            } else {
                $variedades_pedido = DB::table('detalle_import_pedido as d')
                    ...
                    ->get();
                $ids_variedad_pedidos = $variedades_pedido->pluck('id_variedad')->toArray();
                $variedades_ot = DB::table('detalle_import_pedido as d')
                    ->join('import_pedido as ped', 'ped.id_import_pedido', '=', 'd.id_import_pedido')
                    ...
                    ->where('ped.estado', 1)
                    ->where('ped.fecha', $fecha)
                    ->whereNotIn('do.id_variedad', $ids_variedad_pedidos)
                    ->get();
                $ids_variedad_pedidos_ot = $variedades_ot->pluck('id_detalle_import_pedido')->toArray();
                $variedades_pre_ot = DB::table('detalle_import_pedido as d')
                    ->join('import_pedido as ped', 'ped.id_import_pedido', '=', 'd.id_import_pedido')
                    ...
                    ->where('ped.estado', 1)
                    ->where('ped.fecha', $fecha)
                    ->whereNotIn('pdo.id_variedad', $ids_variedad_pedidos)
                    ->whereNotIn('pdo.id_variedad', $ids_variedad_pedidos_ot)
                    ->get();
                $variedades = $variedades_pedido->merge($variedades_ot)->merge($variedades_pre_ot);
            }
            $clientes = DB::table('import_pedido')
                ->select(
                    'id_cliente',
                )->distinct()
                ->where('estado', 1)
                ->where('fecha', $fecha)
                ->get()->pluck('id_cliente')->toArray();

            if($fecha <= '2025-04-06'){ // solo funciona para los historicos hasta de la semana 2515
                foreach ($variedades as $pos_v => $var) {
                    ...
                    foreach ($clientes as $pos_c => $cliente) {
                        if ($dev == 0)
                            dump('fecha: ' . ($pos_f + 1) . '/' . count($fechas) . '; var: ' . ($pos_v + 1) . '/' . count($variedades) . '; cli: ' . ($pos_c + 1) . '/' . count($clientes));
                        ...
                        $detalle_pedidos_ot = DB::table('detalle_import_pedido as d')
                            ...
                            ->where('ped.fecha', $fecha)
                            ->where('ped.id_cliente', $cliente)
                            ->where('do.id_variedad', $var->id_variedad)
                        ...
                        $detalle_pedidos_pre_ot = DB::table('detalle_import_pedido as d')
                            ...
                            ->where('ped.fecha', $fecha)
                            ->where('ped.id_cliente', $cliente)
                            ->where('pdo.id_variedad', $var->id_variedad)
                            ->whereNotIn('d.id_detalle_import_pedido', $ids_detalle_pedidos)
                        ...
                        /* TABLA RESUMEN */
                        $existe = DB::table('resumen_agrogana')
                        ...
                        if (count($existe) == 0) {
                            ...
                            $model = ResumenAgrogana::find($existe[0]->id_resumen_agrogana);
                            $model->tallos_venta = $tallos_venta;
                            $model->tallos_armados = $armados;
            ...
            foreach ($variedades as $pos_v => $var) {
                ...
                $existe = DB::table('resumen_fechas')
                    ->where('id_variedad', $var->id_variedad)
                    ->where('fecha', $fecha)
                    ->get();
                if (count($existe) == 0) {
                    ...
                    $model = ResumenFechas::find($existe[0]->id_resumen_fechas);
                    $model->tallos_comprados = $comprados;
                    $model->tallos_desechados = $desechados;
                    $model->tallos_recibidos = $recibidos;
                    $model->last_update = date('Y-m-d H:i:s');
        ...
        if ($dev == 0) {
            ...
            dump('<<<<< * >>>>> Fin satisfactorio del comando "resumen:update" <<<<< * >>>>>');
            Log::info('<*> DURACION: ' . $time_duration . '  <*>');
            Log::info('<<<<< * >>>>> Fin satisfactorio del comando "resumen:update" <<<<< * >>>>>');
        }
    }
}
