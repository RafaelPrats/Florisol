...
class Variedad extends Model
{
    ...
    {
        return $this->hasMany('\yura\Modelos\ProductoVariedad', 'id_variedad');
    }
    ...
    public function getTotalTallosReceta()
    {
        $tallos = DB::table('detalle_receta')
            ->select(DB::raw('sum(unidades) as cantidad'))
            ->where('id_variedad', $this->id_variedad)
            ->where('defecto', 1)
            ->get()[0]->cantidad;
        return $tallos > 0 ? $tallos : 0;
    }

    public function getTotalPrecioReceta()
    {
        $precio = DB::table('detalle_receta')
            ->select(DB::raw('sum(unidades * precio) as cantidad'))
            ->where('id_variedad', $this->id_variedad)
            ->where('defecto', 1)
            ->get()[0]->cantidad;
        return $precio > 0 ? round($precio, 2) : 0;
    }

    function getDetallesReceta()
    {
        $r = DB::table('detalle_receta')
            ->where('id_variedad', $this->id_variedad)
            ->where('defecto', 1)
            ->get();
        return $r;
    ...
    function getCantidadRecetasBloqueadas()
    {
        $r = DB::table('detalle_receta')
            ->select('numero_receta')->distinct()
            ->where('id_variedad', $this->id_variedad)
