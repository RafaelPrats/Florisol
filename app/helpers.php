...
{
    //$finca = getFincaActiva();
    $disponibles = DB::table('salidas_recepcion as s')
...
function getCantidadDetallesEspecificacionByPedido($pedido)
{
    $r = 0;
...
function getDetallerecetaByItem($receta, $item)
{
    return DB::table('detalle_receta')
        ->where('id_variedad', $receta)
        ->where('id_item', $item)
        ->get()
        ->first();
}

function getTallosVenta($variedad, $fecha)
{
...
function getTallosVentaByVariedad($variedad, $fecha)
{
    /* TALLOS_VENTA y TALLOS_ARMADOS */
    $postcos = DB::table('postco as d')
        ->join('distribucion_postco as r', 'r.id_postco', '=', 'd.id_postco')
        ->select(
            'd.id_postco',
        )->distinct()
        ->where('d.fecha', $fecha)
        ->where('r.id_item', $variedad)
        ->get();
    $ids_postco = $postcos->pluck('id_postco')->toArray();

    $postcos_ot = DB::table('postco as d')
        ->join('ot_postco as o', 'o.id_postco', '=', 'd.id_postco')
        ->join('detalle_ot_postco as do', 'do.id_ot_postco', '=', 'o.id_ot_postco')
        ->select(
            'd.id_postco',
        )->distinct()
        ->where('d.fecha', $fecha)
        ->where('do.id_item', $variedad)
        ->whereNotIn('d.id_postco', $ids_postco)
        ->get();
    $ids_postco_ot = $postcos_ot->pluck('id_postco')->toArray();

    $postcos_armados = DB::table('postco as d')
        ->join('armado_postco as a', 'a.id_postco', '=', 'd.id_postco')
        ->join('detalle_armado_postco as da', 'da.id_armado_postco', '=', 'a.id_armado_postco')
        ->select(
            'd.id_postco',
        )->distinct()
        ->where('d.fecha', $fecha)
        ->where('da.id_item', $variedad)
        ->whereNotIn('d.id_postco', $ids_postco)
        ->whereNotIn('d.id_postco', $ids_postco_ot)
        ->get();
    $postcos = $postcos->merge($postcos_ot)->merge($postcos_armados);

    $tallos_venta = 0;
    foreach ($postcos as $postco) {
        $postco = Postco::find($postco->id_postco);
        ...
        /*if($postco->id_postco == 1501)
            dd($list_ot, $ramos_procesados, $ramos_venta);*/
...
function getRamosDisponiblesByFecha($variedad, $longitud, $fecha)
{
    $postco = Postco::where('id_variedad', $variedad)
        ->where('longitud', $longitud)
        ->where('fecha', $fecha)
        ->get()
        ->first();
    $disponibles = $postco->ramos;
    foreach ($postco->distribuciones as $pos_d => $dist) {
        $inventario = getInventarioDisponibleByVariedadFecha($dist->item, $fecha);
        $posibles = intval($inventario / $dist->unidades);
...
function getTallosVentaByVariedadCliente($variedad, $fecha, $cliente)
{
    ...
    foreach ($postcos as $p) {
        ...
        $ramos_venta = $postco->getRamosByCliente($cliente); // ramos totales del pedido
        $ramos_procesados = $postco->getRamosOtByCliente($cliente);  // ramos procesados mediante OT y Pre-OT
    ...
    return $r['tallos_venta'];
}

function getReportesChatBot()
{
    $usuario = Usuario::find(session('id_usuario'));
    return [
        'v_resumen' => [
            'cambio' => '<b>Tallos vendidos</b>, puedes preguntarme sobre <b>clientes</b>, <b>plantas</b> o <b>variedades</b> relacionadas a los <b>TALLOS</b> vendidos...',
            'bienvenida' => 'Hola ' . explode(' ', $usuario->nombre_completo)[0] . ' 👋 puedes preguntarme sobre <b>clientes</b>, <b>plantas</b> o <b>variedades</b> relacionadas a los <b>TALLOS</b> vendidos.'
        ],
        'v_bouquets' => [
            'cambio' => '<b>Bouquets</b>, puedes preguntarme sobre <b>clientes</b>, <b>plantas</b> o <b>variedades</b> relacionadas a las <b>RECETAS</b> armadas...',
            'bienvenida' => 'Hola ' . explode(' ', $usuario->nombre_completo)[0] . ' 👋 puedes preguntarme sobre <b>clientes</b>, <b>plantas</b> o <b>variedades</b> relacionadas a las <b>RECETAS</b> armadas.'
        ]
    ];
}

function getSchemasChatBot()
{
    return [
        'v_resumen' => "
        TABLE v_resumen ( 
    id_planta INT, (usar esta columna como id unico para cada planta)
    planta_nombre VARCHAR, 
    id_variedad INT, (usar esta columna como id unico para cada variedad)
    variedad_nombre VARCHAR, 
    id_cliente INT, (usar esta columna como id unico para cada cliente)
    cliente_nombre VARCHAR, 
    tallos_armados INT, (esta columna se puede usar para tallos armados o vendidos)
    fecha DATE, 
    semana INT, (ejemplo: la semana 10 del año 2025 esta definida como 2510) 
    anno INT, (ejemplo: puedes buscar directamente el numero del año en esta columna) 
    mes INT 
    )
    
    -- esta tabla se usa tambien para datos de ventas, donde la columna tallos_armados indica la cantidad de tallos vendidos, o tallos usados tambien
    -- usa las columnas id_planta, id_variedad, id_cliente para consultas como: select nombre_cliente from v_resumen where id_cliente not in (select id_cliente from v_resumen where anno = 2025)
    ",
        'v_bouquets' => "
        TABLE v_bouquets (
            id_cliente INT, (usar esta columna como id unico para cada cliente)
            ...
            fecha DATE,
            id_variedad as INT, (usar esta columna como id unico para cada variedad)
            nombre_variedad VARCHAR, (esta columna es el nombre del bouquet, se le llama tambien como receta o ramo)
            longitud INT,
            ramos_pedidos INT,
            ramos_armados INT,
        )

        -- la tabla v_bouquets se usa para saber la cantidad de ramos pedidos, ramos comprados o ramos armados, por cada bouquet
        -- la tabla v_bouquets tambien se usa para los ramos usados o vendidos
        -- usar la tabla v_bouquets cuando se pregunte sobre recetas, bouquets o ramos especificamente
        -- en la tabla v_bouquets un bouquet tambien se le llama como receta, ramo
        -- usa las columnas id_variedad, id_cliente para consultas como: select nombre_cliente from v_bouquets where id_cliente not in (select id_cliente from v_bouquets where YEAR(fecha) = 2025)

        TABLE v_detalle_bouquets(
            fecha DATE,
            id_cliente INT, (usar esta columna como id unico para cada cliente)
            nombre_cliente VARCHAR,
            id_receta as INT, (usar esta columna como id unico para cada receta, bouquet o ramo)
            nombre_receta VARCHAR, (esta columna es el nombre del bouquet, se le llama tambien como receta o ramo)
            longitud INT, (esta columna se usa para saber la medida del ramo, generalmente en centimetros)
            ramos_armados INT, (esta columna se usa para saber los ramos armados de la receta, bouquet o ramo)
            id_planta INT, (usar esta columna como id unico para cada planta)
            nombre_planta VARCHAR, (esta columna es el nombre de la planta a la que pertenece la flor 'item o variedad')
            id_item INT, (usar esta columna como id unico para cada item o tambien llamado como variedad)
            nombre_item VARCHAR, (esta columna es el nombre de la flor o variedad que forma parte de la distribuion del bouquet)
            unidades_por_ramo INT, (esta columna es la cantidad de tallos o unidades de la flor 'nombre_item' en cada ramo del bouquet)
            tallos_totales INT (esta columna es la cantidad de total de tallos de la flor 'nombre_item' en cada ramo del bouquet)
        )

        -- la tabla v_detalle_bouquets representa la el uso de cada flor dentro de los bouquets
        -- usar la tabla v_detalle_bouquets cuando se pregunte por flores, variedad, o planta dentro de cada bouquet, receta o ramo
        -- usa las columnas id_receta, id_cliente, id_planta, id_item para consultas como: select nombre_cliente from v_detalle_bouquets where id_cliente not in (select id_cliente from v_detalle_bouquets where YEAR(fecha) = 2025)
        "
    ];
}
