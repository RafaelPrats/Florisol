...
class ClienteController extends Controller
{
    ...
    {
        return view('adminlte.gestion.postcocecha.clientes.forms.importar_clientes', []);
    }

    public function post_importar_clientes(Request $request)
    {
        ini_set('max_execution_time', env('MAX_EXECUTION_TIME'));
        $valida = Validator::make($request->all(), [
            'file_clientes' => 'required',
        ]);
        $msg = '<div class="alert alert-success text-center">Se ha importado el archivo. Revise su contenido antes de grabar.</div>';
        $success = true;
        if (!$valida->fails()) {
            try {
                $archivo = $request->file_clientes;
                $extension = $archivo->getClientOriginalExtension();
                $nombre_archivo = "upload_clientes." . $extension;
    ...
    public function get_importar_clientes(Request $request)
    {
        try {
            $url = public_path('storage/file_loads/upload_clientes.' . $request->extension);
            $document = IOFactory::load($url);
            $sheet = $document->getActiveSheet()->toArray(null, true, true, true);
            $listado = [];
            foreach ($sheet as $pos_row => $row) {
                if ($pos_row > 1 && $row['A']) {
                    $cliente = Cliente::join('detalle_cliente as d', 'd.id_cliente', '=', 'cliente.id_cliente')
                        ->select('cliente.*', 'd.nombre', 'd.ruc')->distinct()
                        ->where('cliente.codigo', $row['A'])
                        ->where('d.estado', 1)
                        ->get()
                        ->first();
                    $listado[] = [
        ...
        return view('adminlte.gestion.postcocecha.clientes.forms.get_importar_clientes', [
            'listado' => $listado,
        ]);
    }

    public function store_importar_clientes(Request $request)
    {
        try {
            foreach (json_decode($request->data) as $d) {
                DB::beginTransaction();
                if ($d->id != '') {
                    $cliente = Cliente::find($d->id);

                    $detalleActivo = $cliente->detalle();
                    $detalleActivo->estado = 0;
                    $detalleActivo->save();
                } else {
                    $cliente = new Cliente();
                ...
                $objDetalleCliente->nombre                        = mb_strtoupper($d->nombre);
                $objDetalleCliente->ruc                           = $d->ruc;
    ...
    public function buscar_clientes(Request $request)
    {
        ...
        if ($request->busqueda != '') $listado = $listado->Where(function ($q) use ($busqueda) {
            $q->Where('dc.nombre', 'like', '%' . espacios(mb_strtoupper($busqueda)) . '%')
                ->orWhere('dc.correo', 'like', '%' . espacios(mb_strtoupper($busqueda)) . '%')
                ->orWhere('dc.ruc', 'like', '%' . espacios(mb_strtoupper($busqueda)) . '%')
                //->orWhere('pa.nombre', 'like', '%' . espacios(mb_strtoupper($busqueda)) . '%')
